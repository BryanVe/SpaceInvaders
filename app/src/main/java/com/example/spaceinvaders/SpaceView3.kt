...
class SpaceView3(
    ...
    // invaders
    private val invaders = ArrayList<Invader2>()
    ...
    private var playerBullet = Bullet(size.y, 3000f, 40f)
            ...
            for (invader in invaders)
                if (invader.isVisible)
                    if (RectF.intersects(playerBullet.position, invader.position)) {
                        ...
                        playerBullet.isActive = false
                        Invader2.numberOfInvaders--
                        score += 10
                        if (score > highScore) {
                            highScore = score
                        }

                        if (Invader2.numberOfInvaders == 0) {
                            paused = true
                            lives++
                            invaders.clear()
                            bricks.clear()
